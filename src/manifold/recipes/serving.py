...
from manifold.recipes._live_view import _LiveViewPublisher
...
def _run_episode(
    ...
    *,
    ...
    benchmark_name: str,
    recorder: EpisodeRecorder = NO_RECORDER,
    live_view: _LiveViewPublisher | None = None,
) -> EpisodeRecord:
...
def run_benchmark(
    ...
    *,
    ...
    live_view_sensor: str | None = None,
) -> BenchmarkResult:
    ...
    next episode begins. When `live_view_server` is set, `live_view_sensor` selects
    the colour camera published to that dedicated latest-value channel.
    ...
    return _run_connected(
        ...
        live_view_sensor=live_view_sensor,
    )
...
def _run_connected(
    ...
    *,
    ...
    """
    if live_view_server is not None and live_view_sensor is None:
        raise ValueError("live_view_sensor is required when live_view_server is set")
    live_view = None
    if live_view_server is not None and live_view_sensor is not None:
        camera = benchmark.observation_space.camera(live_view_sensor)
        if camera is None:
            raise ValueError(f"benchmark has no camera named {live_view_sensor!r}")
        if camera.modality is not Modality.RGB:
            raise ValueError(f"live-view camera {live_view_sensor!r} must be RGB")
        live_view = _LiveViewPublisher(live_view_server, camera)

    with nullcontext(None) if live_view is None else live_view as publisher:
        return _run_connected_with_live_view(
            benchmark,
            reset_episode,
            step,
...
def _run_connected_with_live_view(
    benchmark: Benchmark,
    reset_episode: Callable[[int], Observation],
    step: Callable[[Action], StepResult],
    *,
    ...
    recorder: EpisodeRecorder,
    on_episode: Callable[[EpisodeRecord], None] | None,
    live_view: _LiveViewPublisher | None,
) -> BenchmarkResult:
    """Drive the connected episodes while the optional live publisher is open."""
    emit = on_event
...
def run_episodes(
    ...
    *,
    ...
    with it. `live_view_server` and `live_view_sensor` configure the optional
...
def run_sharded_benchmark(
    ...
    *,
    ...
    `live_view_sensor` configure the optional latest-value channel as they do for
    `run_benchmark`.
